...
;	MODULE:		Music_Impl (.H)
;
;	PURPOSE:	Implement music capability for LithTech engine using
* 				sound instances
;
...
public:
	...
	public:
		...
		public:
			CSegment()
			{
				...
				m_nDefaultEnact = LTDMEnactNextMeasure;
				m_sound_instances = LTNULL;
				m_num_sound_instances = 0;
			}
			~CSegment()
			{
			...
			unsigned int GetNumInstances() { return m_num_sound_instances; }

			void SetSoundInstances(CSoundInstance** sound_instances, unsigned int num_instances)
			{
				m_sound_instances = sound_instances;
				m_num_sound_instances = num_instances;
			}
		...
		private:
				CSoundInstance** m_sound_instances;
				unsigned int m_num_sound_instances;
		...
		{
		public:
			CSegmentState() { m_sound_instances = LTNULL; m_pSegment = LTNULL; m_current_instance = 0; };
			~CSegmentState() { };
			...
			unsigned int GetCurrentInstance() { return m_current_instance; }
			void AdvanceInstance() { m_current_instance++; }
		...
		private:
			...
			// internal member variable conatining pointer to the sound instance
			CSoundInstance** m_sound_instances;
			unsigned int m_num_sound_instances;
			unsigned int m_current_instance;
		...
		public:
			...
			CSegmentState* Find(const void* pDMSegmentState);

			// add a new segment state (creates, and add to the list)
			void CreateSegmentState(CSoundInstance** pSoundInstances, CSegment* pSegment, unsigned int num_instances);
		...
		public:
			...
			void* GetBand() { return m_pBand; };

			// set the band pointer associated with this item
			void SetBand(void* pSeg) { m_pBand = pSeg; };
			...
			// get the segment pointer associated with this item
			void* GetDMSegment() { return m_pDMSegment; };

			// set the segment pointer associated with this item
			void SetDMSegment(void* pDMSeg) { m_pDMSegment = pDMSeg; };
		...
		public:
			...
			void* GetDMStyle() { return m_pDMStyle; };

			// set the style pointer associated with this item
			void SetDMStyle(void* pStyle) { m_pDMStyle = pStyle; };
		...
		private:
			...
			// internal member variable containing pointer to the direct music style
			void* m_pDMStyle;
		...
		public:
			...
			void* GetDLSBank() { return m_pDLSBank; };

			// set the DLSBank pointer associated with this item
			void SetDLSBank(void* pSeg) { m_pDLSBank = pSeg; };
		...
		public:
			...
			void* GetSegment() const { return m_pDMSegment; }
			void SetSegment(void* pState)
				{
					m_pDMSegment = pState;
				}

			long GetPlayTime() const { return m_nPlayTime; }
			void SetPlayTime(long nTime) { m_nPlayTime = nTime; }
	...
	private:
		...
		// Load Band
		bool LoadBand(void* pStyle, const char* sBandName);
		...
		bool SetReverbParameters(void* pParams);
		...
		bool PlayInstance(CSoundInstance* instance, int reset_prev = 1);

		////////////////////////////////////////////
		// internal static functions

		void HandleSegmentNotification_SegAlmostEnd(CSoundInstance** pSoundInstances);
		void HandleSegmentNotification_SegEnd(CSoundInstance** pSoundInstances);
		...
		void* m_pAudiopathBuffer;

		// the DirectMusic reverb FX for the audiopath
		//LPDIRECTSOUNDFXWAVESREVERB8 m_pReverb; RKNSTUB

		// synthesizer audio parameters
		//DMUS_AUDIOPARAMS m_audioParams; RKNSTUB

		// parameters for the current port
		//DMUS_PORTPARAMS m_portParams; RKNSTUB
		...
		// list of all primary segments that are currently playing
		...
		//DSFXWavesReverb m_ReverbParameters; RKNSTUB

		// contains the FX descriptor for the reverb
		//DSEFFECTDESC	m_ReverbDesc; RKNSTUB

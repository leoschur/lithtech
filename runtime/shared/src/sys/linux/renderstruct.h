...
struct RenderStruct
{
    ...
    // Functions implemented by the render driver.
        ...
        void            (*DeleteSurface)(void* hSurf);

        void            (*GetSurfaceInfo)(void* hSurf, uint32 *pWidth, uint32 *pHeight);

        void*           (*LockSurface)(void* hSurf, uint32& Pitch);
        void            (*UnlockSurface)(void* hSurf);

        // Set transparentColor to OPTIMIZE_NO_TRANSPARENCY to not use transparency.
        bool            (*OptimizeSurface)(void* hSurf, uint32 transparentColor);
        void            (*UnoptimizeSurface)(void* hSurf);
